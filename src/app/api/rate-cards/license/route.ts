...
const schema = z.object({
  ...
  marginPct: z.coerce.number().default(15),
...
export async function POST(req: Request) {
  ...
  const card = await prisma.licenseRateCard.create({
...
export async function DELETE(req: Request) {
  ...
  await prisma.licenseRateCard.delete({ where: { id, organizationId: session.user.organizationId } });
