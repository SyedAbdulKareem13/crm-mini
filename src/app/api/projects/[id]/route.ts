...
const patchSchema = z
  .object({
    ...
    phase: z.object({ id: z.string().min(1), status: z.enum(PHASE_STATUSES) }).optional(),
    deliverable: z.object({ id: z.string().min(1), status: z.enum(DELIVERABLE_STATUSES) }).optional(),
  })
  .refine(
    (d) =>
      d.name !== undefined ||
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Phase status update (scoped through the project).
  if (d.phase) {
    ...
    await prisma.projectPhase.update({ where: { id: phase.id }, data: { status: d.phase.status } });
  ...
  // Deliverable status update (scoped through the project).
  if (d.deliverable) {
    ...
    await prisma.projectDeliverable.update({ where: { id: del.id }, data: { status: d.deliverable.status } });
    // Keep the parent phase status coherent with its deliverables.
  ...
  if (statusChanged || d.phase || d.deliverable || d.name !== undefined) {
    await recordAudit({
      ...
      summary: statusChanged
        ...
        : d.phase
          ? `Phase updated · ${d.phase.status.toLowerCase().replace("_", " ")}`
          : d.deliverable
            ? `Deliverable updated · ${d.deliverable.status.toLowerCase().replace("_", " ")}`
            : `Project updated`,
